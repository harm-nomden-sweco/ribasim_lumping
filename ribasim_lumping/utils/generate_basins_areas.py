...
def create_basin_connections(
    ...
    print(" - create basin connections")
    ...
    conn_gdf = conn_us.merge(conn_ds, left_on='mesh1d_node_id',right_on='mesh1d_node_id',suffixes=('_out','_in'))
    conn_gdf = conn_gdf [['split_type_out','mesh1d_node_id', 'basin_out','basin_in','geom_basin_out','geom_split_node_out','geom_basin_in']]

    # draw connection line via split node
    conn_gdf['geometry'] = conn_gdf.apply(lambda row: LineString([row['geom_basin_out'], row['geom_split_node_out'], row['geom_basin_in']]), axis=1)
...
def get_boundary_basin_connections(
        boundaries: gpd.GeoDataFrame, 
        edges: gpd.GeoDataFrame, 
        basins: gpd.GeoDataFrame,
    ) -> gpd.GeoDataFrame:
    """create boundary-basin connections"""
    print(" - create boundary-basin connections")

    # merge boundaries with edges 
    boundary_basin_connections = boundaries.rename(columns={"geometry":"geometry_boundary"})
    boundary_basin_connections_us = boundary_basin_connections.merge(edges[['start_node_no', 'end_node_no','mesh1d_nEdges', 'basin']], left_on='mesh1d_nNodes', right_on='start_node_no')
    boundary_basin_connections_ds = boundary_basin_connections.merge(edges[['start_node_no','end_node_no','mesh1d_nEdges', 'basin']], left_on='mesh1d_nNodes', right_on='end_node_no')

    # merge with basins for geometry
    # downstream
    boundary_basin_connections_ds = boundary_basin_connections_ds.merge(basins[['basin', 'geometry']], left_on='basin', right_on='basin').rename(columns={"geometry":"geometry_basin"})
    ...
    # same for upstream
    boundary_basin_connections_us = boundary_basin_connections_us.merge(basins[['basin', 'geometry']], left_on='basin', right_on='basin').rename(columns={"geometry":"geometry_basin"})
    ...
    boundary_basin_connections_gdf = boundary_basin_connections_gdf[
        ['boundary_basin_connection_id', 'boundary_id', 'basin', 'boundary_location', 'geometry']
    ]
    return boundary_basin_connections_gdf
...
def create_basins_and_connections_using_split_nodes(
    ...
    boundary_basin_connections = get_boundary_basin_connections(boundaries, edges, basins)

    return basin_areas, basins, areas, nodes, edges, split_nodes, \
        network_graph, basin_connections, boundary_basin_connections
